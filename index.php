...
if (session_status() == PHP_SESSION_NONE) {
  ...
  require_once($_SERVER["DOCUMENT_ROOT"] . "/../Support/basicLib.php");
    ...
    if (isset($_POST["submit"])) {
      //insert new record into db and retain values of last record for user convenience

        $_SESSION['deptContact'] = test_input($_POST["deptContact"]);
        $_SESSION['department'] = test_input($_POST["department"]);
        $_SESSION['locationBldg'] = test_input($_POST["locationBldg"]);
        $_SESSION['locationRoom'] = test_input($_POST["locationRoom"]);
  ...
  <!-- Entry form, if existing record was clicked populate with fille din fields from DB -->
      ...
      <?php
        ...
        <div class="form-group required">
          ...
          <input type="text" class="form-control" tabindex="130" id="deptContact" required name="deptContact" value="<?php echo isset($_SESSION['deptContact']) ? $_SESSION['deptContact'] : '';  ?>"/>
        </div>
        <div class="form-group required">
          <label for="locationBldg">Location - Building</label>
          <input type="text" class="form-control" name="locationBldg" tabindex="140" required value="<?php echo isset($_SESSION['locationBldg']) ? $_SESSION['locationBldg'] : '';  ?>" />
        </div>
        <div class="form-group">
          <label for="locationRoom">Location - Room</label>
          <input type="text" class="form-control" tabindex="150" id="locationRoom" name="locationRoom" value="<?php echo isset($_SESSION['locationRoom']) ? $_SESSION['locationRoom'] : '';  ?>" />
        </div>
        <div class="form-group">
          <label for="dateAcquired">Acquisition Year <em class="text-primary">(if known)</em></label>
          <div class="input-group">
             <input type="text" class="form-control bfh-phone" required tabindex="160" id="dateAcquired" name="dateAcquired" value="<?php echo date("Y") ?>" data-format="dddd" />
          </div>
        </div>
        <div class="form-group required">
